@echo off
echo Setting up HairFastGAN Web API (CPU mode)...
...
REM Apply CPU patches
echo Applying CPU patches to HairFastGAN...
copy /Y HairFastGAN\models\stylegan2\op\fused_act_cpu.py HairFastGAN\models\stylegan2\op\
copy /Y HairFastGAN\models\stylegan2\op\upfirdn2d_cpu.py HairFastGAN\models\stylegan2\op\
if %ERRORLEVEL% neq 0 (
    echo Failed to apply CPU patches.
    goto :error
)
